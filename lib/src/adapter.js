...
// Use {stringToCheck}.startsWith({dataType}) where {dataType} is one of the data types in the array for the respective data type used in MyORM.
...
/** @type {import('@myorm/myorm').AdapterWhereHandler} */
...
/** @type {import('@myorm/myorm').InitializeAdapterCallback<import('mysql2/promise.js').Pool>} */
                ...
                async forDescribe(cmd, args) {
                    ...
                    for(const field in result) {
                        ...
                        set[field] = {
                            ...
                            isUnique: result[field].Key === "UNI" || result[field].Key === "PRI",
                            datatype: type,
                            defaultValue
                        };
                    }
            ...
            return {
                forQuery(data) {
                    ...
                    if (limit && from.length > 1) {
                        ...
                        cmd += `\n\tFROM (SELECT * FROM ${main.table} ${whereInfo.cmd} ${limitStr} ${offsetStr}) AS ${from[0].alias}`;
                ...
                forCount(data) {
                    ...
                    const limitStr = limit != undefined ? `\n\t\tLIMIT ${limit}` : '';
                    const offsetStr = limit != undefined && offset != undefined ? `\n\t\tOFFSET ${offset}` : '';
